...
class FilterSet:
    def __init__(self, name, filters=None) -> None:
        ...
        for fltr in custom_filters:
            missing_inputs = [
                x
                for x in ["name", "wave_eff", "width", "transmission"]
                if x not in list(fltr.keys())
            ]
            if missing_inputs:
                print(
                    f'WARNING! Filter {fltr} is missing the following inputs: "{", ".join(missing_inputs)}"'
                )
            else:
                tmp_throughput, tmp_wave_eff, tmp_fwhm = generate_tophat_throughput(
                    fltr["name"], fltr["wave_eff"], fltr["width"], fltr["transmission"]
                )
    ...
    def set_snr(self, snr_input, quiet=False) -> None:
        if (
            (type(snr_input) == int)
            or (type(snr_input) == float)
            or (type(snr_input) == np.float64)
            ...
            if not quiet:
                for fltr in missing_fltr:
                    print(
                        f'WARNING! "{fltr}" is not included in snr_input; S/N will remain {self.snr_dict[fltr]}.'
                    )
            ...
            if len(snr_input) != len(self.filters):
                raise InstError(
                    "snr_input must be the same length as the number of filters"
                )
            self.snr_dict = {
                self.filters[i]: snr_input[i] for i in range(len(snr_input))
            }
            self.snr = np.array(list(self.snr_dict.values()))
        else:
            raise InstError(
                "snr_input must be a single value, a dictionary, a list, or a 1D numpy array"
            )
        self.snr = np.array(list(self.snr_dict.values()))
